                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <div class="nav-tabs-custom no-shadow">
                            ...
                            <br>
                            <form action="{{ route('admin.shop.categories.update', $category->slug) }}" method="post" enctype="multipart/form-data" data-parsley-validate="" class="form-horizontal form-label-left">
                                @csrf
                                @method('PATCH')
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <select name="filter_id[]" id="filters" class="form-control" multiple="multiple">
                                                    @foreach($filters as $key => $filter)
                                                        <option value="{{ $key }}"{{ (in_array($key, $selectedFilters) ? ' selected' : '') }}>{{ $filter }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <select name="manufacturer_id[]" id="manufacturers" class="form-control" multiple="multiple">
                                                    @foreach($manufacturers as $key => $manufacturer)
                                                        <option value="{{ $key }}"{{ (in_array($key, old('manufacturer_id') ?? $selectedManufacturers) ? ' selected' : '') }}>{{ $manufacturer }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
